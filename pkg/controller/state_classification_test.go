...
func TestPodInfo_DetectStateInconsistency(t *testing.T) {
	tests := []struct {
		...
		{
			name:                "inconsistent - should be master but marked as initial",
			memgraphRole:        "main",
			currentState:        INITIAL,
			replicaCount:        2,
		...
		{
			name:                "inconsistent - should be replica but marked as master",
			memgraphRole:        "replica",
			currentState:        MASTER,
			replicaCount:        0,
			expectInconsistency: true,
		...
		{
			name:                "inconsistent - should be master but marked as replica",
			memgraphRole:        "main",
