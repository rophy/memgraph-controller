...
def test_cluster_topology(cluster_ready):
    ...
    sync_count = len([r for r in replicas_result['records'] if r['sync_mode'] == 'sync'])
    async_count = len([r for r in replicas_result['records'] if r['sync_mode'] == 'async'])
    ...
    sync_replica = next((r for r in replicas_result['records'] if r['sync_mode'] == 'sync'), None)
    async_replica = next((r for r in replicas_result['records'] if r['sync_mode'] == 'async'), None)
    
    sync_name = sync_replica['name'] if sync_replica else 'unknown'
    async_name = async_replica['name'] if async_replica else 'unknown'
    
    log_info(f"📊 Topology verified: Main role confirmed, Sync={sync_name}, Async={async_name}")
    ...
    assert sync_pod in ["memgraph-ha-0", "memgraph-ha-1"], f"Sync replica should be pod-0 or pod-1, got: {sync_pod}"
    
    log_success(f"✅ Cluster topology validated: {sync_pod}(sync), {async_pod}(async)")
...
def test_data_write_gateway(cluster_ready, unique_test_id):
    ...
    write_query = f"CREATE (n:TestNode {{id: '{unique_test_id}', value: '{test_value}', timestamp: datetime()}}) RETURN n.id;"
...
def test_multiple_operations():
    ...
    log_success(f"✅ Multiple operations verified: {multi_count} test nodes created and verified")
...
def test_cluster_health_queries():
    ...
    # Test SHOW REPLICAS
    replicas_result = memgraph_query_via_client("SHOW REPLICAS;")
    # Note: replicas_result might be empty if no replicas are registered yet, that's okay
    ...
    count_result = memgraph_query_via_client("MATCH (n) RETURN count(n) as total_nodes;")
