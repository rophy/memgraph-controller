...
def test_network_partition_failover(network_isolation_manager):
    ...
    for attempt in range(24):  # 24 * 5 = 120 seconds
        try:
            ...
            controller_log_filepath = get_pod_logs(
                controller_pod, since_time=test_start_time)
            failover_events = detect_failover_in_log_file(
                controller_log_filepath)
    ...
    controller_pod = get_controller_pod()
    final_controller_log_filepath = get_pod_logs(
        controller_pod, since_time=test_start_time)
    failover_events = detect_failover_in_log_file(
        final_controller_log_filepath)
