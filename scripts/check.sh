...
run_memgraph_query() {
    ...
    kubectl exec "$pod_name" -n memgraph -c memgraph -- bash -c "echo \"$query\" | mgconsole --username=memgraph" 2>/dev/null || echo "❌ Failed to query $pod_name"
}
...
for pod in "${pods[@]}"; do
    if kubectl get pod "$pod" -n memgraph &>/dev/null; then
        ...
        role=$(kubectl exec "$pod" -n memgraph -c memgraph -- bash -c 'echo "SHOW REPLICATION ROLE;" | mgconsole --username=memgraph' 2>/dev/null | grep -o '"[^"]*"' | tr -d '"')
