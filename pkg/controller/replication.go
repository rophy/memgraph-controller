...
func (c *MemgraphController) verifyReplicaSyncStatus(ctx context.Context, masterPod *PodInfo, replicaName string) error {
	// Check if replica exists in master's replica list
	...
	if replicaInfo == nil {
		return fmt.Errorf("replica %s not registered with master", replicaName)
	}
...
func (c *MemgraphController) verifyAsyncToSyncPromotion(ctx context.Context, clusterState *ClusterState, promotedReplica string) error {
	...
	// Re-query master's replica information to get updated status
	currentMaster := clusterState.CurrentMaster
	masterPod := clusterState.Pods[currentMaster]
	
	// Refresh master pod's replica information
	if refreshedPod, err := c.RefreshPodInfo(ctx, masterPod.Name); err != nil {
		log.Printf("Warning: Could not refresh master replica info: %v", err)
	} else {
		*masterPod = *refreshedPod
	}
	...
	return fmt.Errorf("replica %s not found in master's replica list after promotion", promotedReplica)
}
...
func (c *MemgraphController) detectSyncReplicaHealth(ctx context.Context, clusterState *ClusterState) error {
	currentMaster := clusterState.CurrentMaster
	if currentMaster == "" {
		return fmt.Errorf("no master available for SYNC replica health monitoring")
	}
	
	masterPod, exists := clusterState.Pods[currentMaster]
	if !exists {
		return fmt.Errorf("master pod %s not found for SYNC replica health monitoring", currentMaster)
	}
	...
	if !c.isPodHealthyForMaster(syncPod) {
		log.Printf("🚨 SYNC replica pod %s is unhealthy", syncReplicaName)
		return c.handleSyncReplicaFailure(ctx, clusterState, syncReplicaName)
	}
	
	// Check replica health from master's perspective
...
func (c *MemgraphController) handleSyncReplicaFailure(ctx context.Context, clusterState *ClusterState, failedSyncReplica string) error {
	...
	if syncPod, exists := clusterState.Pods[failedSyncReplica]; exists && c.isPodHealthyForMaster(syncPod) {
	...
	if err := c.memgraphClient.DropReplicaWithRetry(ctx, masterPod.BoltAddress, failedReplicaName); err != nil {
...
func (c *MemgraphController) configurePodAsSyncReplica(ctx context.Context, clusterState *ClusterState, podName string) error {
	currentMaster := clusterState.CurrentMaster
	if currentMaster == "" {
		return fmt.Errorf("no master available for SYNC replica configuration")
	}
